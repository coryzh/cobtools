...
@click.argument(
    "ra", type=click.FloatRange(min=0, max=360), metavar="ra",
    help=(
...
@click.argument(
    "dec", type=click.FloatRange(min=-90, max=90), metavar="dec",
    help=(
        "Declination in decimal degrees. "
        "Must be in the range [-90, 90] degrees."
    )
)
@click.argument(
    "pmra", type=float, metavar="pmra",
    help=(
        "Proper motion in right ascension (including the cos(dec) factor) "
        "in mas/yr."
    )
)
@click.argument(
    ...
    help=(
        "Error in proper motion in right ascension in mas/yr."
    )
)
@click.argument(
    "pmdec", type=float, metavar="pmdec",
    help=(
...
@click.argument(
    ...
    help=(
        "Error in proper motion in declination in mas/yr."
    )
)
@click.argument(
    "dist", type=click.FloatRange(min=0, min_open=True), metavar="dist",
    help=(
        "Distance in kpc. Must be a positive value."
    )
)
@click.argument(
...
@click.argument(
    "rv", type=click.FloatRange(min=-3e5, max=3e5), metavar="rv",
    help=(
